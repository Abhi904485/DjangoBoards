...
class TopicTests(TestCase):
    def setUp(self):
        ...
        self.url = reverse('boards:board_topics', kwargs={'pk': self.board.pk})
        self.view = resolve('/boards/{}/'.format(self.board.pk))
    ...
    def test_topic_view_not_found_status_code(self):
        self.url = reverse('boards:board_topics', kwargs={'pk': 99})
        self.response = self.client.get(self.url)
    ...
    def test_new_topic_create_button_success_in_topics_page(self):
        self.assertContains(self.response, reverse('boards:new_topic', kwargs={'pk': self.board.pk}))

    def test_new_topic_create_button_not_found_in_topics_page(self):
        self.assertNotContains(self.response, reverse('boards:new_topic', kwargs={'pk': 99}))
